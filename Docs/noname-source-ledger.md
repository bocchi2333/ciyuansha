...
| 上游文件 | 本地子系统 | 使用方式 |
| --- | --- | --- |
| `apps/core/noname/library/element/gameEvent.ts` | `CiyuanSha.GameCore/Events`、`Engine/GameEngine.cs` | 将父子事件、next/after 队列、Before/Begin/Resolve/End/After 生命周期、阶段指令和确定性触发顺序重写为纯 C# |
...
| `apps/core/card/standard.js` | `Data/Packs/standard-cards`、`Engine/GameEngine.cs` | 对照标准牌、装备、花色点数和规则说明 |
| `apps/core/card/extra.js` | `Data/Packs/military-cards`、`Engine/GameEngine.cs` | 对照属性杀、酒、铁索连环、火攻及军争装备 |
...
上述本地实现均为按 GPL 源码公开机制和接口语义独立重写的 C# 实现，没有逐行复制 JavaScript；即使是独立重写，本项目仍整体按 GPL-3.0-only 治理。若以后出现直接翻译或实质改写，必须新增一行，
